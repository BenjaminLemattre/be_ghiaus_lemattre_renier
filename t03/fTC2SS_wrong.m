...
f1([r0' rC']) = f; % rearange in order of f-sources   
in = [find(b) size(A,1)+find(f1)]; %effective inputs [temp_on_branches flow_in_nodes]
...
y1([r0' rC']) = y; % rearange in order of f-sources 
Ds = Dx(find(y1),in);